// _tools/scrape_set_en.js
// Scrapes English cards from en.onepiece-cardgame.com
// Uso: node _tools/scrape_set_en.js <set_id> <series_number> <category> [producto]
// Ejemplo: node _tools/scrape_set_en.js OP-17 569117 BOOSTER
...
if (!setId || !seriesNum || !category) {
  console.error('Uso: node _tools/scrape_set_en.js <set_id> <series_number> <category> [producto]');
  process.exit(1);
}
...
const htmlPath = path.join(process.env.TEMP || '/tmp', 'opcg_en_scrape_' + setId + '.html');

// ── 1. Download HTML ──
console.log('[' + setId + ' EN] Descargando HTML...');
execSync('curl.exe -s -X POST "https://en.onepiece-cardgame.com/cardlist/" -d "series=' + seriesNum + '" -o "' + htmlPath + '"', { stdio: 'pipe' });
...
console.log('[' + setId + ' EN] HTML descargado: ' + (raw.length / 1024).toFixed(0) + ' KB, ' + (resultsCount || '?') + ' resultados');
...
while ((dlMatch = dlRegex.exec(raw)) !== null) {
  ...
  cards.push({
    ...
    producto: cat,
    category: category,
    card_image: 'assets/images/onepiece/en/' + dirSafe + '/' + imgFile,
    effect: effect,
    power: power,
    counter: counter,
    attribute: attribute,
...
console.log('[' + setId + ' EN] Parseadas: ' + cards.length + ' cartas (' + cards.filter(c => c.is_parallel).length + ' paralelas)');

// ── 3. Download images ──
const outDir = path.join('assets/images/onepiece/en', setId);
...
async function processImages() {
  ...
  for (let i = 0; i < cards.length; i += batchSize) {
    ...
    process.stdout.write('\r[' + setId + ' EN] Imagenes: ' + done + '/' + cards.length);
  }
...
(async () => {
  ...
  console.log('[' + setId + ' EN] Agregado al master. Total: ' + master.cards.length + ' cartas');
