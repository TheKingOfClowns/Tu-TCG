...
Deno.serve(async (req: Request) => {
  ...
  console.log("[sync-binder-cards] Starting sync...");

  try {
    const { binder_id, cards, user_id } = await req.json();

    if (!binder_id || !cards || !Array.isArray(cards)) {
      console.error("[sync-binder-cards] Invalid input:", { binder_id, cards });
      return new Response(JSON.stringify({ error: "Invalid input" }), { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } });
    }

    console.log(`[sync-binder-cards] Syncing binder ${binder_id} with ${cards.length} cards for user ${user_id}`);
